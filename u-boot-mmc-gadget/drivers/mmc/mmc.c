...
//printf("sd_change_freq 5 %x %x\n", scr[0], scr[1]);
	mmc->scr[0] = __be32_to_cpu(scr[0]);
	mmc->scr[1] = __be32_to_cpu(scr[1]);

printf("0 mmc version 0x%x\n", mmc->version);
	...
	{
		printf("high speed not supported\n");
		return 0;
	}
	/*
...
int mmc_send_if_cond(struct mmc *mmc)
{
	...
	{
		printf("mmc_send_if_cond unusable\n");
		return UNUSABLE_ERR;
	}
	else
		mmc->version = SD_VERSION_2;
...
printf("mmc_init\n");
	...
	if (err)
	{
		printf("mmc_send_if_cond error\n");
		return err;
	}
printf("3 mmc version 0x%x\n", mmc->version);

	/* Now try to get the SD card's operating condition */
	err = sd_send_op_cond(mmc);

printf("4 mmc version 0x%x\n", mmc->version);
